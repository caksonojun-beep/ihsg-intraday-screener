"""
IHSG Stock Screener - BPJS Detector Module
============================================
BPJS = Beli Pagi Jual Sore (Buy Morning Sell Afternoon)
Detects stocks suitable for day trading strategy.
"""
...
class BPJSSignal(Enum):
    """Enumeration of BPJS signal types."""
    STRONG_BPJS = "STRONG BPJS"
    BPJS = "BPJS"
    WATCH = "WATCH"
    AVOID = "AVOID"
...
class BPJSDetector:
    """
    BPJS Detector for morning-to-afternoon trading strategy.
    ...
    def _determine_signal(self, score: int) -> BPJSSignal:
        if score >= self.score_thresholds['STRONG_BPJS']:
            return BPJSSignal.STRONG_BPJS
        elif score >= self.score_thresholds['BPJS']:
            return BPJSSignal.BPJS
        elif score >= self.score_thresholds['WATCH']:
            return BPJSSignal.WATCH
        else:
            return BPJSSignal.AVOID
    ...
    def calculate_bpjs_score(
        ...
        volatility_score = self._calculate_volatility_score(bpjs_volatility)
        ...
        total_score = (
            ...
            volatility_score * (self.weights['bpjs_volatility'] / 100) +
        ...
        return BPJSStock(
            ...
            bpjs_volatility=bpjs_volatility,
            pre_closing_momentum=pre_closing_momentum,
            bpjs_score=total_score,
            signal=signal,
    ...
    def analyze_batch(self, stocks_data: List[Dict]) -> List[BPJSStock]:
        results = []
        for stock in stocks_data:
            try:
                bpjs_stock = self.calculate_bpjs_score(
                    ...
                    bpjs_volatility=stock.get('bpjs_volatility', 2.0),
                    pre_closing_momentum=stock.get('pre_closing_momentum', 0)
                )
                results.append(bpjs_stock)
            except Exception:
                continue
        return results

    def filter_bpjs_stocks(
        self,
        stocks: List[BPJSStock],
        min_score: int = 60,
        min_change: float = 0.5
    ) -> List[BPJSStock]:
        filtered = [s for s in stocks if s.bpjs_score >= min_score]
        if min_change > 0:
            filtered = [s for s in filtered if s.change_pct >= min_change]
        return filtered

    def get_top_bpjs_picks(
        self,
        stocks: List[BPJSStock],
        n: int = 10,
        signal: Optional[BPJSSignal] = None
    ) -> List[BPJSStock]:
        filtered = stocks
        if signal:
            filtered = [s for s in filtered if s.signal == signal]
        sorted_stocks = sorted(filtered, key=lambda x: x.bpjs_score, reverse=True)
        return sorted_stocks[:n]

    def to_dataframe(self, stocks: List[BPJSStock]) -> pd.DataFrame:
        data = []
        for stock in stocks:
            data.append({
                'Ticker': stock.ticker,
